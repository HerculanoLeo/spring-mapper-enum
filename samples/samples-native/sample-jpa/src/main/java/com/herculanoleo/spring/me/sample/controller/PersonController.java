...
public class PersonController {
    ...
    @GetMapping
    public ResponseEntity<Collection<Person>> findAll(PersonSearchRequest requestEntity) {
    ...
    @PostMapping
    public ResponseEntity<Person> register(@RequestBody PersonRegisterRequest requestEntity) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.register(requestEntity));
    }

    @PutMapping("{id}")
    public ResponseEntity<Person> update(@PathVariable("id") UUID id, @RequestBody PersonUpdateRequest requestEntity) {
